...
<script>
  ...
  import DispatchAdminNav from "@dispatchAdmin-components/partials/NavComponent";
  import DispatchAdminHeader from "@dispatchAdmin-components/partials/HeaderComponent";
  import DispatchAdminFooter from "@admin-components/partials/FooterComponent";

  export default {
    name: "DispatchAdminApp",
    data: () => ({
      freshLoad: true,
      isLoading: true
    }),
    ...
    methods: {
      logoutUser(msg = "Logging you out....") {
        ...
        axios.post("/dispatch-admins/logout").then(rsp => {
          location.reload();
        });
      },
      toggleLoadState() {
