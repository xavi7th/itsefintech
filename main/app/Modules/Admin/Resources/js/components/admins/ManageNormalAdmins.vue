<template>
  <main>
    <page-header pageTitle="Manage Normal Admins"></page-header>
    <div class="content">
      <!-- table basic -->
      <div class="card">
        ...
        <div class="card-body">
                          ...
                          <div class="table-responsive">
                            <div class="flex j-c-between bd bg-light py-30 px-30">
                              <div class="flex-sh-0 ln-18">
                                <div class="fs-16 fw-500 text-success">Normal Admin</div>
                      ...
                      <div class="card-body py-0">
                        <div class="card-row">
                          ...
                          <button
                            ...
                            @click="updateNormalAdminPermissions"
                          >Save changes</button>
                        </div>
                      </div>
                    </div>
          ...
          <div class="modal modal-right fade" id="modal-admin" tabindex="-1">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title">Enter Normal Admin Details</h4>
                  <button type="button" class="close" data-dismiss="modal">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
...
<script>
  import {
    adminViewNormalAdmins,
    adminNormalAdminPermissions,
    adminCreateNormalAdmin,
    adminDeleteNormalAdmin,
    adminRestoreNormalAdmin,
    adminSuspendNormalAdmin
  ...
  export default {
    ...
    created() {
      axios.get(adminViewNormalAdmins).then(({ data: { users } }) => {
        this.users = users;
    ...
    methods: {
      ...
      showPermModal(userDetails) {
        this.userDetails = userDetails;
        axios
          .get(adminNormalAdminPermissions(userDetails.id))
      ...
      updateNormalAdminPermissions() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "updating admin permissions ..."
        });
        axios
          .put(adminNormalAdminPermissions(this.userDetails.id), {
      ...
      deleteNormalAdmin() {
        ...
        axios
          .delete(adminDeleteNormalAdmin(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      suspendNormalAdmin() {
        ...
        axios
          .put(adminSuspendNormalAdmin(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      restoreNormalAdmin() {
        ...
        axios
          .put(adminRestoreNormalAdmin(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      createNormalAdmin() {
        this.$validator.validateAll().then(result => {
          if (!result) {
            ...
            axios
              .post(adminCreateNormalAdmin, { ...this.details })
