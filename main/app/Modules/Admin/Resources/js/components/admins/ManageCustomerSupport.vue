<template>
  <main>
    <page-header pageTitle="Manage Customer Supports"></page-header>
    <div class="content">
      <!-- table basic -->
      <div class="card">
        <div class="card-title">
          <button
            ...
            data-toggle="modal"
            data-target="#modal-customer-support"
          >Create Customer Support</button>
        </div>
        <div class="card-body">
                          ...
                          <div class="table-responsive">
                            <div class="flex j-c-between bd bg-light py-30 px-30">
                              <div class="flex-sh-0 ln-18">
                                <div class="fs-16 fw-500 text-success">Customer Support</div>
                      ...
                      <div class="card-body py-0">
                        <div class="card-row">
                          ...
                          <button
                            ...
                            @click="updateCustomerSupportPermissions"
                          >Save changes</button>
                        </div>
                      </div>
                    </div>
          ...
          <div class="modal modal-right fade" id="modal-customer-support" tabindex="-1">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title">Enter Customer Support Details</h4>
                  <button type="button" class="close" data-dismiss="modal">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
...
<script>
  import {
    adminViewCustomerSupports,
    adminCustomerSupportPermissions,
    adminCreateCustomerSupport,
    adminDeleteCustomerSupport,
    adminRestoreCustomerSupport,
    adminSuspendCustomerSupport
  } from "@admin-assets/js/config";
  import PreLoader from "@admin-components/misc/PageLoader";
  export default {
    name: "ManageCustomerSupports",
    data: () => ({
      users: [],
      userDetails: {},
    ...
    created() {
      axios.get(adminViewCustomerSupports).then(({ data: { users } }) => {
        this.users = users;
    ...
    methods: {
      ...
      showPermModal(userDetails) {
        this.userDetails = userDetails;
        axios
          .get(adminCustomerSupportPermissions(userDetails.id))
      ...
      updateCustomerSupportPermissions() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "updating customer support's permissions ..."
        });
        axios
          .put(adminCustomerSupportPermissions(this.userDetails.id), {
      ...
      deleteCustomerSupport() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "Deleting customer support's account ..."
        });
        axios
          .delete(adminDeleteCustomerSupport(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      suspendCustomerSupport() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "suspending customer support's account ..."
        });
        axios
          .put(adminSuspendCustomerSupport(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      restoreCustomerSupport() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "restoring customer support's account ..."
        });
        axios
          .put(adminRestoreCustomerSupport(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      createCustomerSupport() {
        this.$validator.validateAll().then(result => {
          if (!result) {
            ...
            axios
              .post(adminCreateCustomerSupport, { ...this.details })
