<template>
  <main>
    <page-header pageTitle="Manage AccountOfficers"></page-header>
    <div class="content">
      <!-- table basic -->
      <div class="card">
        ...
        <div class="card-body">
                          ...
                          <div class="table-responsive">
                            <div class="flex j-c-between bd bg-light py-30 px-30">
                              <div class="flex-sh-0 ln-18">
                                <div class="fs-16 fw-500 text-success">Account Officer</div>
                      ...
                      <div class="card-body py-0">
                        <div class="card-row">
                          ...
                          <button
                            ...
                            @click="updateAccountOfficerPermissions"
                          >Save changes</button>
                        </div>
                      </div>
                    </div>
          ...
          <div class="modal modal-right fade" id="modal-account-officer" tabindex="-1">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title">Enter Account Officer Details</h4>
                  <button type="button" class="close" data-dismiss="modal">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
...
<script>
  import {
    adminViewAccountOfficers,
    adminAccountOfficerPermissions,
    adminCreateAccountOfficer,
    adminDeleteAccountOfficer,
    adminRestoreAccountOfficer,
    adminSuspendAccountOfficer
  } from "@admin-assets/js/config";
  import PreLoader from "@admin-components/misc/PageLoader";
  export default {
    name: "ManageAccountOfficers",
    data: () => ({
      users: [],
      userDetails: {},
    ...
    created() {
      axios.get(adminViewAccountOfficers).then(({ data: { users } }) => {
        this.users = users;
    ...
    methods: {
      ...
      showPermModal(userDetails) {
        this.userDetails = userDetails;
        axios
          .get(adminAccountOfficerPermissions(userDetails.id))
      ...
      updateAccountOfficerPermissions() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "updating account officer's permissions ..."
        });
        axios
          .put(adminAccountOfficerPermissions(this.userDetails.id), {
      ...
      deleteAccountOfficer() {
        ...
        axios
          .delete(adminDeleteAccountOfficer(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      suspendAccountOfficer() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "suspending account officer's account ..."
        });
        axios
          .put(adminSuspendAccountOfficer(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      restoreAccountOfficer() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "restoring account officer's account ..."
        });
        axios
          .put(adminRestoreAccountOfficer(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      createAccountOfficer() {
        this.$validator.validateAll().then(result => {
          if (!result) {
            ...
            axios
              .post(adminCreateAccountOfficer, { ...this.details })
