<template>
  <main>
    <page-header pageTitle="Manage Dispatch Admins"></page-header>
    <div class="content">
      <!-- table basic -->
      <div class="card">
        <div class="card-title">
          <button
            ...
            data-toggle="modal"
            data-target="#modal-dispatch-admin"
          >Create Dispatch Admin</button>
        </div>
        <div class="card-body">
                          ...
                          <div class="table-responsive">
                            <div class="flex j-c-between bd bg-light py-30 px-30">
                              <div class="flex-sh-0 ln-18">
                                <div class="fs-16 fw-500 text-success">Dispatch Admin</div>
                      ...
                      <div class="card-body py-0">
                        <div class="card-row">
                          ...
                          <button
                            ...
                            @click="updateDispatchAdminPermissions"
                          >Save changes</button>
                        </div>
                      </div>
                    </div>
          ...
          <div class="modal modal-right fade" id="modal-dispatch-admin" tabindex="-1">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title">Enter Dispatch Admin Details</h4>
                  <button type="button" class="close" data-dismiss="modal">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
...
<script>
  import {
    adminViewDispatchAdmins,
    adminDispatchAdminPermissions,
    adminCreateDispatchAdmin,
    adminDeleteDispatchAdmin,
    adminRestoreDispatchAdmin,
    adminSuspendDispatchAdmin
  } from "@admin-assets/js/config";
  import PreLoader from "@admin-components/misc/PageLoader";
  export default {
    name: "ManageDispatchAdmins",
    data: () => ({
      users: [],
      userDetails: {},
    ...
    created() {
      axios.get(adminViewDispatchAdmins).then(({ data: { users } }) => {
        this.users = users;
    ...
    methods: {
      ...
      showPermModal(userDetails) {
        this.userDetails = userDetails;
        axios
          .get(adminDispatchAdminPermissions(userDetails.id))
      ...
      updateDispatchAdminPermissions() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "updating dispatch admin's permissions ..."
        });
        axios
          .put(adminDispatchAdminPermissions(this.userDetails.id), {
      ...
      deleteDispatchAdmin() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "Deleting dispatch admin's account ..."
        });
        axios
          .delete(adminDeleteDispatchAdmin(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      suspendDispatchAdmin() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "suspending dispatch admin's account ..."
        });
        axios
          .put(adminSuspendDispatchAdmin(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      restoreDispatchAdmin() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "restoring dispatch admin's account ..."
        });
        axios
          .put(adminRestoreDispatchAdmin(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      createDispatchAdmin() {
        this.$validator.validateAll().then(result => {
          if (!result) {
            ...
            axios
              .post(adminCreateDispatchAdmin, { ...this.details })
