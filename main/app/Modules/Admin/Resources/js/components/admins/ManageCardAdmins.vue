<template>
  <main>
    <page-header pageTitle="Manage Card Admins"></page-header>
    <div class="content">
      <!-- table basic -->
      <div class="card">
        ...
        <div class="card-body">
                          ...
                          <div class="table-responsive">
                            <div class="flex j-c-between bd bg-light py-30 px-30">
                              <div class="flex-sh-0 ln-18">
                                <div class="fs-16 fw-500 text-success">Card Admin</div>
                      ...
                      <div class="card-body py-0">
                        <div class="card-row">
                          ...
                          <button
                            ...
                            @click="updateCardAdminPermissions"
                          >Save changes</button>
                        </div>
                      </div>
                    </div>
          ...
          <div class="modal modal-right fade" id="modal-account-officer" tabindex="-1">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title">Enter Card Admin Details</h4>
                  <button type="button" class="close" data-dismiss="modal">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
...
<script>
  import {
    adminViewCardAdmins,
    adminCardAdminPermissions,
    adminCreateCardAdmin,
    adminDeleteCardAdmin,
    adminRestoreCardAdmin,
    adminSuspendCardAdmin
  } from "@admin-assets/js/config";
  import PreLoader from "@admin-components/misc/PageLoader";
  export default {
    name: "ManageCardAdmins",
    data: () => ({
      users: [],
      userDetails: {},
    ...
    created() {
      axios.get(adminViewCardAdmins).then(({ data: { users } }) => {
        this.users = users;
    ...
    methods: {
      ...
      showPermModal(userDetails) {
        this.userDetails = userDetails;
        axios
          .get(adminCardAdminPermissions(userDetails.id))
      ...
      updateCardAdminPermissions() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "updating account officer's permissions ..."
        });
        axios
          .put(adminCardAdminPermissions(this.userDetails.id), {
      ...
      deleteCardAdmin() {
        ...
        axios
          .delete(adminDeleteCardAdmin(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      suspendCardAdmin() {
        ...
        axios
          .put(adminSuspendCardAdmin(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      restoreCardAdmin() {
        ...
        axios
          .put(adminRestoreCardAdmin(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      createCardAdmin() {
        this.$validator.validateAll().then(result => {
          if (!result) {
            ...
            axios
              .post(adminCreateCardAdmin, { ...this.details })
