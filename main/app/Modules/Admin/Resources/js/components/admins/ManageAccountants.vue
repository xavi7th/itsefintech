    ...
    <div class="content">
      ...
      <div class="card">
        ...
        <div class="card-body">
                          ...
                          <div class="table-responsive">
                            <div class="flex j-c-between bd bg-light py-30 px-30">
                              <div class="flex-sh-0 ln-18">
                                <div class="fs-16 fw-500 text-success">Accountant</div>
                      ...
                      <div class="card-body py-0">
                        <div class="card-row">
                          ...
                          <button
                            ...
                            @click="updateAccountantPermissions"
                          >Save changes</button>
                        </div>
                      </div>
                    </div>
          ...
          <div class="modal modal-right fade" id="modal-accountant" tabindex="-1">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title">Enter Accountant Details</h4>
                  <button type="button" class="close" data-dismiss="modal">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
...
<script>
  import {
    adminViewAccountants,
    adminAccountantPermissions,
    adminCreateAccountant,
    adminDeleteAccountant,
    adminRestoreAccountant,
    adminSuspendAccountant
  ...
  export default {
    ...
    created() {
      axios.get(adminViewAccountants).then(({ data: { users } }) => {
        this.users = users;
    ...
    methods: {
      ...
      showPermModal(userDetails) {
        this.userDetails = userDetails;
        axios
          .get(adminAccountantPermissions(userDetails.id))
      ...
      updateAccountantPermissions() {
        ...
        BlockToast.fire({
          text: "updating accountant's permissions ..."
        });
        axios
          .put(adminAccountantPermissions(this.userDetails.id), {
      ...
      deleteAccountant() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "Deleting accountant's account ..."
        });
        axios
          .delete(adminDeleteAccountant(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      suspendAccountant() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "suspending accountant's account ..."
        });
        axios
          .put(adminSuspendAccountant(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      restoreAccountant() {
        this.sectionLoading = true;
        BlockToast.fire({
          text: "restoring accountant's account ..."
        });
        axios
          .put(adminRestoreAccountant(this.userDetails.id))
          .then(({ status }) => {
            if (status === 204) {
              Toast.fire({
                title: "Success",
      ...
      createAccountant() {
        this.$validator.validateAll().then(result => {
          if (!result) {
            ...
            axios
              .post(adminCreateAccountant, { ...this.details })
